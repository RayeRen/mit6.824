...
func TestReElection2AForNTimes(t *testing.T) {
	for i := 0; i < 10; i++ {
		TestReElection2A(t)
	}
}
...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("%d disconnect", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	DPrintf("%d connect", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf("%d disconnect", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("%d disconnect", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("%d connect", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("%d connect", leader2)
	cfg.checkOneLeader()
...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		if xindex != index {
			t.Fatalf("got LogIndex %v but expected %v", xindex, index)
		}
	}
...
func TestFailNoAgree2B(t *testing.T) {
	...
	if index != 2 {
		t.Fatalf("expected LogIndex 2, got %v", index)
	}
	...
	// among their own ranks, forgetting LogIndex 2.
	// or perhaps
	leader2 := cfg.checkOneLeader()
	...
	if index2 < 2 || index2 > 3 {
		t.Fatalf("unexpected LogIndex %v", index2)
	}
...
func TestRejoin2B(t *testing.T) {
	...
	// new leader commits, also for LogIndex=2
	cfg.one(103, 2)
	...
	for try := 0; try < 5; try++ {
		...
		for i := 1; i < iters+1; i++ {
			...
			if ix, ok := cmd.(int); ok == false || ix != cmds[i-1] {
				...
				t.Fatalf("wrong value %v committed for LogIndex %v; expected %v\n", cmd, starti+i, cmds)
			}
		}
